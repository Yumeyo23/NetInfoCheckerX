...
- 本程序不是绝对的**专业工具**，而是**专业向工具**，提供的功能均以个人日常需求开发。请君免与任何同类软件比较。
- 作者承诺，**程序不收集，不上传用户的任何信息到作者** (使用到的各个API来源会在程序里标注出来)
- 作者**非计算机相关专业，非网络相关专业，无编程背景一切皆为兴趣**，只了解易语言，**为重制查询器才开始了解C#，大部分代码均为AI代替完成**(当然设计思路是自己的，代码也有自己完成的部分, 人力/AI比例可能三七?二八?)。
    故代码丑陋，初期不乐意开源，还请见谅。想喷的伙计轻点儿(x。欢迎大佬帮助开发/优化，有需要大佬联系可以提供源码
- 本程序是**C# NET Framework 4.7.2 WinForm窗口程序**，且作者**无任何升/换框架/跨平台欲望**。
- 理由：1. 我菜；
    2. 生活中大多数电脑都是Windows，NET4.7.2编译体积小巧且兼容性好，除外观相对较丑外，无需任何依赖即可运行，更满足便携要求，打包后10MB左右体积可以完成后面的一坨功能，~~不像NatTypeTester那样100MB只能测NAT一件事~~
    3. 手上只有手机的情况，多数时候只需最常用的找AP、多出口、NAT测试就可以了，直接ip测漏等网站/软件，或使用[查询器多出口在线版](https://yumeyo23.github.io/NetInfoChecker/checker-web.html)，一站式找到**没有电脑的大多数情况下**需要的功能 
- 感谢陪伴awa

## 灵感/致谢
- NAT类型测试灵感源自于[NatTypeTester](https://github.com/HMSBridge/NatTypeTester)(但未使用其代码)，并加入了大量的优化功能：例如**显示debug信息、设置端口模式/起手、列出可用网卡指定IP测试**  ~~(其实我也有点忍不了一个纯NAT测试要100MB)~~
...
- [x] 本机硬件检测    **//25.11.20开工，完工**
- [x] IPERF测速工具GUI     **//25.11.21开工，完工**
- [x] RFC3489/5780NAT测试     **//25.11.21开工，25.11.22完工**
- [x] TCP/UDP Tracert+自定义网卡   **//25.11.22开工，完工**
- [x] 本机所有网卡, 可快速复制IPV6/打开默认网关 **//25.11.28开工，完工**
- [x] 快速Ping/Tracert/Nslookup剪贴板  **//25.11.29开工，完工**
- [x] 一键/Renew [ipcfg/release/renew/flushdns] **//25.11.29开工，完工**
- [x] 快速跳转控制面板  **//25.11.29开工，完工**
- [x] IPV6有效期  **//25.11.29开工，完工**
- [ ] 多出口测试精简版（名站+三大+教育+测漏）
- [ ] 多出口测试完整版（名站+三大+教育+测漏+双栈+UDP）
- [ ] 记录/读取国内IP
- [ ] 查国内IP在记录中出现次数
- [ ] 手动查IP/MAC
- [ ] 延迟测试[CS完美平台/全球网测骨干节点]
- [ ] 自由查询
- [ ] 悬浮信息时间显示
- [ ] WakeOnLan
